...
import { OHLCTooltip, MovingAverageTooltip, BollingerBandTooltip } from "react-financial-charts/lib/tooltip";
import { ema, sma, bollingerBand } from "react-financial-charts/lib/indicator";
...
class CandleStickChartWithBollingerBandOverlay extends React.Component {
	...
	render() {
		...
		const { onDestroy, type, data: initialData, width, ratio } = this.props;
...
function showAct2(e){
	console.log("showAct", e);
	showDestroy = false;
	console.log("showDestroy", showDestroy);
	//test = "bbbbb";
...
const { testText, showChart } = this.state;
		return (
			...
			>
				...
				{ 
					...
					<LineSeries id="line1" yAccessor={sma20.accessor()} stroke={sma20.stroke()} clip={false} />	
				...
				>
					<YAxis axisAt="left" orient="left" ticks={5} tickFormat={format(".2s")} zoomEnabled={false} />
